...
(defn _tile-inc-pattern [pattern increment]
  (if (> increment 0)
    (if (pattern.endswith "*")
        pattern
        (+ "_" (get pattern (slice 1 -1))))
    (if (pattern.startswith "*")
        pattern
        (+ (get pattern (slice 0 -2)) "_"))))
...
(defn geometry [geom-str [frame "monitor"]]
  ...
  (_insist-on-geometry
    (ap-when (.group geom-parts "w")
      (*current-gdk-window*.resize (_hdimension_to-pixels it)
                                   (_vdimension_to-pixels (.group geom-parts "h"))))
    (ap-when (.group geom-parts "x")
      (*current-gdk-window*.move (_hdimension_to-pixels it)
                                 (_vdimension_to-pixels (.group geom-parts "y")))))
  (return True))
...
(defn monitor-height []
  ...
  (. (*gdk-disp*.get_monitor_at_window *current-gdk-window*)
     (get_geometry)
     height))
...
(defn tile [[v-pattern "*"]
  ...
  (setv (. _*tile-patterns* [(window_xid)]) [v-pattern h-pattern])
  (_print-when-verbose f"TILE {w-px}x{h-px}+{x-px}+{y-px}")
  (_insist-on-geometry
    (*current-gdk-window*.move x-px y-px)
    (*current-gdk-window*.resize w-px h-px)))

(defn tile-at [direction]
  ...
  (case direction
    ...
    "full"         (return (tile "*"    "*"  ))
    else           (do (print f"Invalid direction for tile : '{direction}'")
                       (return False))))

(defn tile-move [direction]
  (if (in (window_xid) _*tile-patterns*)
    (setv [v-pattern h-pattern] (. _*tile-patterns* [(window_xid)]))
